        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                ...
                <div className="p-4 overflow-y-auto flex-1">
                    <p className="text-gray-500 mb-4 bg-blue-50 p-3 rounded-lg text-sm border-l-4 border-blue-500">
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="flex justify-between items-start mb-2">
                                        <div className={`text-xs px-2 py-0.5 rounded font-bold ${draft.source === 'local' ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>
                                    ...
                                    <h4 className="font-bold text-gray-800 truncate mb-1">{data.customer_name || 'Tanpa Nama'}</h4>

                                    <div className="flex justify-between items-center text-sm">
                                        <span className="text-gray-500">{data.items?.length || 0} Item</span>
                                        <span className="font-bold text-gray-900">Rp {data.total?.toLocaleString('id-ID')}</span>
                                    </div>

                                    {data.table_number && (
                                        <div className="mt-2 text-xs text-gray-500">
                                            Meja: <span className="font-semibold text-gray-700">{data.table_number}</span>
                                        </div>
                                    )}
                                </div>
                            );
                ...
                <div className="p-4 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
                    <button
                        onClick={onClose}
                        className="px-6 py-2.5 rounded-lg text-gray-700 font-bold hover:bg-gray-200 transition-colors"
                    >
                        Batal
                    </button>
                    <button
                        ...
                        className="px-6 py-2.5 rounded-lg bg-primary-600 text-white font-bold hover:bg-primary-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all shadow-lg hover:shadow-xl active:scale-95 flex items-center gap-2"
                    >
