<div class="pb-32 bg-gray-50 min-h-screen font-sans">
    <!-- Search Bar (Sticky below layout header) -->
    <!-- top-16 because layout header height is roughly 4rem (64px) due to mt-16 in layout -->
    <div class="sticky top-16 z-30 bg-white border-b border-gray-200 px-4 py-3 shadow-sm">
    ...
    <!-- Categories -->
    <!-- top-[125px] = 64px (header) + ~61px (search bar) -->
    <div class="sticky top-[125px] z-20 bg-white border-b border-gray-100 py-2 shadow-sm">
    ...
    <!-- Featured Products (Upselling) - Grid Layout -->
    @if($selectedCategoryId === 'all' && empty($search) && $featuredProducts->isNotEmpty())
    <div class="px-5 pt-4 pb-4 bg-gradient-to-b from-yellow-50/30 to-transparent">
        <div class="flex items-center gap-2 mb-3">
            ...
            <span class="ml-auto text-[10px] text-gray-500 font-medium">{{ $featuredProducts->count() }} menu</span>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3">
            ...
            <div wire:key="featured-{{ $product->id }}"
                class="bg-white rounded-2xl p-3 shadow-md border-2 border-yellow-200 relative group overflow-hidden flex flex-col">
                <!-- Featured Badge -->
                <div class="absolute top-2 right-2 z-10">
                    <span class="bg-yellow-500 text-white text-[8px] px-1.5 py-0.5 rounded-full font-bold uppercase shadow-sm">
                        ⭐ Top
                    </span>
                </div>
                ...
                <h3 class="font-bold text-gray-800 text-xs leading-snug line-clamp-2 mb-1">
                    {{ $product->name }}
                </h3>
                ...
                <button wire:click="addToCart({{ $product->id }})" wire:loading.attr="disabled"
                    class="w-full bg-yellow-50 hover:bg-yellow-100 text-yellow-700 hover:text-yellow-800 py-2 rounded-lg font-bold text-xs flex items-center justify-center gap-1 border border-yellow-200 active:scale-95 transition-all">
