<div x-data="kioskData()" x-init="initKiosk()"
    ...
    <div class="relative z-10 text-center space-y-8 w-full max-w-6xl px-4">

        {{-- Header & Clock --}}
        <div class="flex flex-col md:flex-row justify-between items-center w-full px-8">
            <div class="text-left">
                <h1
                    class="text-3xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-indigo-400">
                    Mesin Absensi AI
                </h1>
                <p class="text-slate-400 text-sm" x-text="message"></p>
            </div>
            <div class="text-right">
                <div class="text-xl font-light text-indigo-300 mb-1">{{ $currentDate }}</div>
                <div class="text-5xl font-black tracking-tighter text-white font-mono" x-text="time">{{ $currentTime }}
                </div>
            </div>
        </div>

        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8 items-center h-full">
            ...
            <div class="relative group w-full max-w-xl mx-auto">
                ...
                <div
                    ...
                    <div x-show="!isCameraOpen"
                        ...
                        <svg class="w-16 h-16 opacity-50 mb-4 animate-pulse" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z">
                            </path>
                        </svg>
                        <span x-text="modelLoaded ? 'Klik Start untuk Memulai' : 'Memuat Model AI...'"></span>

                        <button x-show="modelLoaded" @click="startCamera()"
                            class="mt-6 px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-full font-bold shadow-lg transition transform hover:scale-105">
                            START CAMERA
                        </button>
                    </div>
                    ...
                    <div x-show="isCameraOpen && detectedName !== '...'"
                        ...
                        <div class="bg-black/60 backdrop-blur-md border border-white/20 px-6 py-2 rounded-full text-white font-semibold shadow-xl flex items-center gap-2 transition-all"
            ...
            <div class="flex flex-col justify-center space-y-6 max-w-md mx-auto w-full">

                {{-- Status Card --}}
                <div
                    class="bg-slate-800/50 backdrop-blur-sm p-8 rounded-3xl border border-slate-700/50 text-center min-h-[200px] flex flex-col justify-center">
                    <template x-if="!matchedEmployeeId">
                        <div>
                            <h2 class="text-slate-500 text-xl font-medium mb-2">Menunggu Wajah...</h2>
                            <div class="flex justify-center mt-4 space-x-2">
                                <div class="w-2 h-2 bg-slate-600 rounded-full animate-bounce"
                    ...
                    <template x-if="matchedEmployeeId">
                        <div class="animate-fade-in-up">
                            <div
                                class="mx-auto w-24 h-24 rounded-full overflow-hidden border-4 border-indigo-500/30 mb-4 bg-slate-700">
                                <img :src="matchedPhoto || `https://ui-avatars.com/api/?name=${detectedName}&background=random`"
                                    class="w-full h-full object-cover">
                            </div>
                            <h2 class="text-3xl font-bold text-white mb-2" x-text="detectedName"></h2>
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <button @click="performAction('in')"
                        ...
                        class="group relative flex flex-col items-center justify-center p-6 rounded-2xl transition-all duration-300 transform border
                        ...
                        <div class="mb-2 p-3 rounded-full transition
