<div class="h-full flex flex-col md:flex-row bg-gray-50 overflow-hidden">
    ...
    {{-- 💰 KIRI: DAFTAR PRODUK --}}
    {{-- ========================= --}}
    <div class="md:w-2/3 w-full h-full flex flex-col border-r border-gray-200 bg-white shadow-sm">
        {{-- Header Produk --}}
        <div class="px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-blue-50 to-indigo-50">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-xl font-bold text-gray-900">Menu Produk</h1>
                    <p class="text-sm text-gray-600 mt-1">Pilih produk untuk ditambahkan ke keranjang</p>
                </div>
            ...
            <div class="flex space-x-2 overflow-x-auto pb-1 scrollbar-thin scrollbar-thumb-gray-300">
                @foreach ($categories as $category)
                    <button wire:click="setCategory('{{ $category }}')"
                        class="cursor-pointer flex-shrink-0 px-4 py-2.5 rounded-lg text-sm font-medium transition-all duration-200 border
        ...
        <div class="flex-1 overflow-y-auto p-6">
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5 gap-4 auto-rows-[13rem]">
                @forelse ($products as $product)
                    <div wire:click="addProduct({{ $product->id }})"
                        ...
                        <div class="text-center flex-1 flex flex-col justify-between w-full">
                            <div>
                                <h3 class="text-sm font-semibold text-gray-900 line-clamp-2 leading-tight mb-1">
                                    {{ $product->name }}
                                </h3>
                ...
                @empty
                    <div class="col-span-full text-center py-12">
                        ...
                        <p class="text-gray-500 text-sm">Tidak ada produk ditemukan untuk kategori ini.</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    {{-- 🧺 KANAN: KERANJANG --}}
    {{-- ========================= --}}
    <div class="md:w-1/3 w-full h-full flex flex-col bg-white shadow-lg">
        ...
        <div class="p-6 bg-gradient-to-r from-green-50 to-emerald-50 border-b border-gray-100">
            <div class="flex items-center justify-between mb-4">
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Kelola pesanan customer</p>
                </div>
                <div class="text-right">
            ...
            <div class="bg-white rounded-xl border border-gray-200 p-4 space-y-4">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <button 
                            ...
                            class="w-full cursor-pointer px-3 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 border
                                {{ $orderType === 'Dine In'
                ...
                <div>
                    ...
                    <input 
                        ...
                        class="w-full bg-gray-50 border border-gray-300 rounded-lg py-2.5 px-3 text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 focus:outline-none transition"
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-3">
                            ...
                            <div class="flex items-center justify-between">
                                <div class="flex items-center space-x-2">
                                    <button wire:click="updateQuantity({{ $index }}, {{ $item['quantity'] - 1 }})"
                                        class="cursor-pointer w-8 h-8 flex items-center justify-center bg-gray-100 hover:bg-gray-200 rounded-lg text-gray-600 font-semibold transition">
                                        −
                                    </button>
            ...
            @empty
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-500 text-sm">Pilih produk dari menu untuk memulai pesanan</p>
                </div>
            @endforelse
        </div>
            ...
            <div class="p-6 border-b border-gray-100">
                ...
                <div class="flex space-x-3">
                    ...
                    <button 
                        ...
                        class="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl text-sm font-semibold shadow-sm hover:shadow-md transition">
                        Terapkan
                    </button>
                </div>
                @if ($discountMessage)
                    <p class="text-sm mt-2 font-medium {{ $discountApplied ? 'text-green-600' : 'text-red-600' }}">
                        {{ $discountMessage }}
                    </p>
                @endif
            </div>
            ...
            <div class="p-6 space-y-3">
                ...
                <div class="border-t border-gray-200 pt-3">
                    <div class="flex justify-between text-lg font-bold text-gray-900">
                        <span>Total</span>
                        <span class="text-xl">Rp{{ number_format($finalTotal, 0, ',', '.') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="p-6 border-t border-gray-100 bg-gray-50 space-y-3">
                <div class="grid grid-cols-2 gap-3">
                    <button wire:click="saveSale"
                        class="cursor-pointer w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-xl font-bold text-sm shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02]">
                        💾 SIMPAN TRANSAKSI
                    </button>
                    <button wire:click="openPaymentModal({{ $saleId }})" 
                            class="cursor-pointer w-full bg-green-600 hover:bg-green-700 text-white py-4 rounded-xl font-bold text-sm shadow-lg hover:shadow-xl transition-all duration-200 transform hover:scale-[1.02]"
                            {{ !$saleId ? 'disabled' : '' }}>
                        💵 PROSES PEMBAYARAN
                    </button>
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <button wire:click="openLoadModal"
                        class="cursor-pointer bg-yellow-500 hover:bg-yellow-600 text-white py-3 rounded-xl font-semibold text-sm shadow-sm hover:shadow-md transition flex items-center justify-center space-x-2">
                    ...
                    <button wire:click="cancelSale"
                        class="cursor-pointer bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 rounded-xl font-semibold text-sm transition flex items-center justify-center space-x-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
