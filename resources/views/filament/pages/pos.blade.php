...
<div x-data="posLayout()" x-init="initLayout()" @resize.window.debounce.200ms="calculatePerPage()"
    ...
    <script>
        ...
        // 🖨️ Client-Side Print Handler (Electron Bridge)
        window.addEventListener('print-order-client', event => {
            const jobs = event.detail.jobs;
            console.log('🖨️ Client-Side Print received:', jobs);

            if (window.electronAPI && typeof window.electronAPI.printOrder === 'function') {
                console.log('🚀 Sending to Electron Printer...');
                window.electronAPI.printOrder(jobs)
                    .then(res => console.log('✅ Print sent to Electron:', res))
                    .catch(err => console.error('❌ Electron print failed:', err));
            } else {
                console.warn('⚠️ Electron API (window.electronAPI.printOrder) not found. Skipping client-side print.');
            }
        });

        // ⚡ Optimized Alpine Component for Product Item
        // Prevents inline object creation for every row, reducing memory & initialization cost
        document.addEventListener('alpine:init', () => {
            Alpine.data('productItem', (productId) => ({
                clickCount: 0,
                timeout: null,
                ...
                addToCart() {
                    ...
                    this.timeout = setTimeout(() => {
                        if (this.clickCount > 0) {
                            this.$wire.addMoreProduct(productId, this.clickCount);
                            this.clickCount = 0;
                        }
                    }, 300); // 300ms debounce for combo clicks
                }
            }));
        });
    </script>
    ...
    <div id="products-section"
        ...
        <div class="p-3 bg-white/95 border-b border-gray-100 flex-shrink-0 z-20 shadow-sm sticky top-0">
            ...
            <div class="space-y-2">
                ...
                <div class="relative w-full group transition-all duration-300">
                    ...
                    <input wire:model.live.debounce.400ms="searchQuery" id="product-search-input" type="text"
        ...
        <div class="flex-1 overflow-y-auto p-2 sm:p-4 bg-slate-50 relative" id="product-grid-container">
            ...
            {{-- OPTIMIZED: No polling, use event-based updates instead --}}
            <div
                ...
                <div wire:key="product-{{ $product->id }}" @if($product->is_available) x-data="productItem({{ $product->id }})" @click="animateFlyToCart($event); addToCart()"
...
<div id="cart-section"
    ...
    <div class="p-3 bg-white/95 border-b border-gray-100 flex-shrink-0 z-20 shadow-sm">
        ...
        <div class="space-y-2">
            ...
            <div class="relative group z-30">
                ...
                <div
                    class="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-xl border border-gray-100 overflow-hidden z-50 max-h-48 overflow-y-auto">
                    @foreach($foundMembers as $member)
                    <div wire:click="selectMember({{ $member['id'] }})"
                        class="p-2 hover:bg-violet-50 cursor-pointer flex justify-between items-center border-b border-gray-50 last:border-0 transition-colors">
                        <div>
                        ...
                        <div class="text-right">
                            <span
                                class="text-[9px] bg-violet-100 text-violet-700 px-1 py-0.5 rounded font-bold">{{ $member['tier']['name'] ?? 'M' }}</span>
                        </div>
                    </div>
                    @endforeach
                </div>
        ...
        <div wire:key="cart-item-{{ $index }}"
            ...
            <div class="flex items-start gap-2">
                ...
                <div x-data="{ 
                                                                                            ...
                                                                                            updateQty(val) {
                                                                                                this.qty = val;
                                                                                                $wire.updateQuantity({{ $index }}, val).then(() => {
                                                                                                    // Sync back from server after update
                                                                                                    this.qty = $wire.items[{{ $index }}].quantity;
                                                                                                });
                                                                                            }
                                                                                        }"
                ...
                <div class="flex-1 min-w-0 flex flex-col justify-between">
                    <div class="flex justify-between items-start gap-2">
                        <h3 class="font-bold text-slate-700 text-xs leading-tight line-clamp-2 cursor-pointer hover:text-violet-600"
                            wire:click="openEditNotes({{ $index }})">
                            {{ $item['name'] }}
                        </h3>
                    ...
                    @if(!empty($item['notes']))
                    <div
                        class="mt-1 px-1.5 py-0.5 bg-amber-50 border border-amber-100 rounded text-[9px] text-amber-800 truncate">
    ...
    {{-- Ringkasan & Aksi --}}
    <div
        class="border-t border-gray-200 bg-white flex-shrink-0 z-20 pb-safe shadow-[0_-4px_20px_-10px_rgba(0,0,0,0.05)]">
        {{-- Input Diskon --}}
        ...
        <div class="p-3 pt-0 border-t border-gray-100 bg-slate-50/80 space-y-2">
            ...
            <button wire:click="openPaymentModal" wire:loading.attr="disabled"
                ...
                {{ !$saleId ? 'disabled' : '' }}>
                <svg wire:loading.remove wire:target="openPaymentModal" class="w-5 h-5" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z" />
                </svg>
                <svg wire:loading wire:target="openPaymentModal" class="animate-spin h-5 w-5 text-white"
            ...
            <div class="grid grid-cols-2 gap-2">
                <button wire:click="saveSale" wire:loading.attr="disabled"
                    ...
                    <svg wire:loading.remove wire:target="saveSale" class="w-4 h-4" fill="none" viewBox="0 0 24 24"
                        stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <button wire:click="reprintOrder" wire:loading.attr="disabled"
                    ...
                    <svg wire:loading wire:target="reprintOrder" class="w-4 h-4 animate-spin" fill="none"
                        stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15">
                        </path>
        ...
        <div class="inline-block w-full max-w-md my-8 text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-16 sm:align-middle"
            ...
            <div class="px-6 py-3 bg-gray-100 border-t border-gray-200 rounded-b-lg flex justify-end space-x-3">
                ...
                <button wire:click="saveItemNotes" wire:loading.attr="disabled"
                    ...
                    <span class="flex items-center">
                        <svg wire:loading.remove wire:target="saveItemNotes" class="w-4 h-4 mr-2" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4"></path>
                        </svg>
                        <svg wire:loading wire:target="saveItemNotes" class="w-4 h-4 mr-2 animate-spin" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15">
                            </path>
...
<div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <!-- Modal Box -->
    <div
        class="relative bg-white rounded-2xl shadow-2xl w-full max-w-5xl h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">

        <!-- Header -->
        <div
            class="px-6 py-4 bg-white border-b border-gray-100 flex justify-between items-center flex-shrink-0 z-10">
        ...
        <div class="flex-1 overflow-hidden flex flex-col md:flex-row">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
                    ...
                    <div wire:click="toggleSelectSale({{ $saleId }})"
                        ...
                        <div class="absolute top-3 right-3 z-10">
                            @if($isTarget)
                            <span
                                class="inline-flex items-center gap-1 bg-green-500 text-white text-[10px] font-bold px-2 py-1 rounded-full shadow-sm">
                                <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            @elseif($isSelected)
                            <div
                                class="w-6 h-6 bg-purple-500 rounded-full flex items-center justify-center text-white shadow-sm transition-transform scale-100">
                                <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                        d="M5 13l4 4L19 7"></path>
                                </svg>
                            ...
                            @else
                            <div
                                class="w-6 h-6 border-2 border-gray-300 rounded-full group-hover:border-purple-400 transition-colors">
                            </div>
                            @endif
                        </div>
            ...
            <!-- Right Panel: Summary -->
            <div
                class="w-full md:w-96 bg-white flex flex-col border-l border-gray-200 z-20 shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)]">
                <div class="p-6 flex-1 flex flex-col">
                    ...
                    <div class="flex-1 space-y-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <div
                                ...
                                <div class="flex items-center justify-between text-sm group">
                                    ...
                                    <span
                                        class="text-xs text-gray-400">Rp{{ number_format($s['final_total'] ?? 0, 0, ',', '.') }}</span>
                                </div>
                                @endif
                                @endforeach
                        ...
                        <div>
                            ...
                            <div
                                class="bg-green-50 border-2 border-green-500 border-dashed rounded-xl p-4 relative overflow-hidden">
                                ...
                                <div class="relative z-10">
                                    ...
                                    <div class="text-sm text-green-800">{{ $target['customer_name'] ?? '...' }}
                                    </div>
                                </div>
                            </div>
                            @else
...
<div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <!-- Modal Box -->
    <div
        class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200">

        <!-- Header -->
        <div
            class="px-6 py-4 bg-gradient-to-r from-violet-600 to-indigo-600 flex justify-between items-center text-white">
            <div class="flex items-center gap-3">
            ...
            <button wire:click="$set('showRewardModal', false)"
        ...
        <div class="p-4 bg-slate-50 max-h-[60vh] overflow-y-auto space-y-4">
                ...
                <div class="flex gap-2">
                    <div class="flex-1">
                        ...
                        <div class="relative mt-1">
                            <input type="number" wire:model.live.debounce.500ms="pointRedemptionAmount"
                                ...
                                placeholder="0">
                            <div
                                class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none text-xs text-gray-400 font-bold">
                                Pts
                            </div>
                        </div>
                        <div class="flex justify-between items-center mt-1">
                    ...
                    <div class="flex-1">
                        ...
                        <div
                            ...
                            Rp
                            {{ number_format((int) $pointRedemptionAmount * ($settings->loyalty_point_value ?? 1)) }}
                        </div>
            ...
            <div>
                ...
                <div class="space-y-3">
                    ...
                    @endphp
                    <div
                        class="bg-white p-3 rounded-xl border border-gray-100 shadow-sm flex justify-between items-center {{ !$canRedeem ? 'opacity-60 grayscale' : '' }}">
                        <div class="flex items-center gap-3">
                            <div class="w-12 h-12 bg-slate-100 rounded-lg overflow-hidden">
                                <img src="{{ $reward->product->image ? asset('storage/' . $reward->product->image) : 'https://placehold.co/100x100?text=Reward' }}"
                            ...
                            <div>
                                ...
                                <div
                                    class="mt-1 inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-amber-50 text-amber-600 border border-amber-100">
