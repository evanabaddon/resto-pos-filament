<div class="animate-fade-in">
    ...
    <div class="flex justify-center mb-8">
        <div
            class="inline-flex p-1 bg-gray-100 rounded-xl dark:bg-gray-800 shadow-sm border border-gray-200 dark:border-gray-700">
            @foreach(['kitchen' => '👨‍🍳 ' . __('messages.kitchen'), 'bar' => '🍸 ' . __('messages.bar'), 'retail' => '🛍️ ' . __('messages.retail'), 'ready' => '✅ ' . __('messages.ready')] as $key => $label)
        ...
        @if($batches->isEmpty())
            <div class="flex flex-col items-center justify-center h-96 text-center animate-fade-in">
                <div class="relative mb-6 group">
                    <div
                        class="absolute inset-0 bg-green-100 rounded-full blur-xl opacity-50 blur-anim dark:bg-green-900/30">
                    </div>
                    <x-heroicon-o-check-circle
                        class="relative w-24 h-24 text-green-500 transition-transform duration-500 transform group-hover:scale-110" />
                </div>
                <h3 class="text-2xl font-bold text-gray-800 dark:text-gray-100">
                    {{ $activeTab === 'ready' ? __('messages.kds_empty_ready') : __('messages.kds_all_done') }}</h3>
                <p class="mt-2 text-gray-500 dark:text-gray-400">{{ __('messages.kds_good_job') }}</p>
            </div>
        @else
            <!-- Masonry-like Grid -->
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
                @foreach($batches as $batch)
                    ...
                    <div
                        class="flex flex-col relative w-full bg-white dark:bg-gray-800 rounded-2xl shadow-lg border-l-4 {{ $borderColor }} overflow-hidden hover:shadow-xl transition-shadow duration-300">

                        <!-- Header -->
                        <div class="px-5 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800">
                            <div class="flex justify-between items-start mb-2">
                                <span
                                    class="text-2xl font-black text-gray-800 dark:text-white tracking-tight leading-none truncate"
                                    title="{{ $sale->customer_name ?: __('messages.guest') }}">
                                    {{ $sale->customer_name ?: __('messages.guest') }}
                                </span>
                                <span
                                    class="text-sm font-bold text-primary-600 dark:text-primary-400 uppercase tracking-wider mt-1">
                                    {{ $sale->table_number ? __('messages.table_number') . $sale->table_number : ($sale->order_type ?? __('messages.order')) }}
                                </span>
                                <div class="flex flex-col items-end text-right">
                                    <span
                                        class="text-lg font-bold font-mono {{ $isLongWait && $activeTab !== 'ready' ? 'text-red-500 animate-pulse' : 'text-gray-600 dark:text-gray-300' }}">
                                        {{ $batch->created_at->format('H:i') }}
                                    </span>
                                    <span
                                        class="text-xs font-medium text-gray-400 bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full mt-1">
                                        {{ $batch->created_at->locale(app()->getLocale())->diffForHumans(null, true, true) }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex-1 p-4 space-y-3">
                            @foreach($batch->items as $item)
                                ...
                                @endphp
                                <div
                                    class="group relative flex flex-col p-3 rounded-xl border {{ $statusColors }} transition-all duration-200">
                                    <div class="flex justify-between items-start w-full">
                                        <div class="flex-1 pr-2">
                                            <div class="flex items-baseline gap-2">
                                                <span class="font-black text-xl">{{ (float) $item->quantity }}x</span>
                                                <span
                                                    class="font-bold text-base leading-tight">{{ $item->product_name ?? $item->product->name ?? __('messages.no_name') }}</span>
                                            </div>
                                            @if($item->notes)
                                                <div
                                                    class="mt-1 flex items-start gap-1 text-red-500 text-xs font-semibold bg-red-50 dark:bg-red-900/20 p-1.5 rounded-lg inline-block">
                                            ...
                                            @else
                                                @if($item->status === 'pending')
                                                    <button wire:click="markItemStatus({{ $item->id }}, 'cooking')"
                                                        ...
                                                        title="{{ __('messages.start_cooking') }}">
