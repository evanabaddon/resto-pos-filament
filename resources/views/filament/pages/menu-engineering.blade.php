<x-filament-panels::page>
    <div class="space-y-6" x-data="{ activeFilter: 'ALL' }">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div>
                <h2 class="text-2xl font-bold tracking-tight text-gray-950 dark:text-white">
                    {{ __('messages.menu_engineering_title') }}</h2>
                <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('messages.menu_engineering_desc') }}</p>
            </div>

            @if($lastGeneratedAt)
                <div class="flex items-center gap-4">
                    <x-filament::button wire:click="exportPdf" icon="heroicon-m-document-arrow-down" color="gray" size="sm"
                        wire:loading.attr="disabled">
                        {{ __('messages.export_pdf') }}
                    </x-filament::button>

                    <div
                        class="flex items-center gap-2 text-xs text-gray-400 bg-gray-100 dark:bg-gray-800 px-3 py-1.5 rounded-full">
                        <x-heroicon-m-clock class="w-4 h-4" />
                        {{ __('messages.analyzed_at') }}: {{ $lastGeneratedAt }}
                        <button wire:click="generateMatrix" wire:loading.attr="disabled"
                            ...
                            <x-heroicon-m-arrow-path
                                ...
                                wire:loading.class="animate-spin" wire:target="generateMatrix" />
                        </button>
                    </div>
                </div>
            @endif
        ...
        @if(!$matrixData || empty($matrixData['items']))
            <div
                class="flex flex-col items-center justify-center py-12 bg-white dark:bg-gray-900 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">
                <x-heroicon-o-chart-bar-square class="w-16 h-16 text-gray-300 mb-4" />
                <h3 class="text-lg font-medium text-gray-900 dark:text-white">{{ __('messages.no_metrics_analysis') }}</h3>
                <p class="text-sm text-gray-500 mb-6 text-center max-w-xs">{{ __('messages.no_metrics_desc') }}</p>
                <x-filament::button wire:click="generateMatrix" size="lg" icon="heroicon-m-sparkles"
                    wire:loading.attr="disabled">
                    <span wire:loading.remove wire:target="generateMatrix">{{ __('messages.generate_matrix') }}</span>
                    <span wire:loading wire:target="generateMatrix">{{ __('messages.analyzing_matrix') }}</span>
                </x-filament::button>
            </div>
        @else
                    <!-- AI Insight Section -->
                        ...
                        <div
                            ...
                            <div class="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h3 class="font-bold text-indigo-900 dark:text-indigo-400">
                                        {{ __('messages.ai_strategic_insights') }}</h3>
                            ...
                            <div class="prose prose-sm dark:prose-invert max-w-none">
                                <p class="text-gray-700 dark:text-gray-300 italic">
                                    "{{ $aiAdvice['overall_analysis'] ?? __('messages.ai_analyzing') }}"</p>

                                @if(isset($aiAdvice['top_priorities']))
                                    <div class="mt-4 flex flex-wrap gap-2">
                                        @foreach($aiAdvice['top_priorities'] as $priority)
                                            <span
                                                class="bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-400 px-3 py-1 rounded-lg text-xs font-bold border border-indigo-200 dark:border-indigo-500/30">
                                                📍 {{ $priority }}
                                            </span>
                                        @endforeach
                                    </div>
                                ...
                                @foreach(collect($aiAdvice['strategic_advice'] ?? [])->take(4) as $advice)
                                        <div class="bg-white/50 dark:bg-gray-800/50 p-4 rounded-xl border border-white dark:border-gray-700">
                                            <div class="flex justify-between items-start mb-2">
                                                <span
                                                    class="text-sm font-bold text-gray-900 dark:text-white">{{ $advice['product_name'] }}</span>
                                        ...
                                        default => 'bg-rose-100 text-rose-700'
                                                    ...
                                                    {{ match ($advice['category'] ?? '') {
                                        'UNIT UNGGULAN' => __('messages.star'),
                                        'UNIT ANDALAN' => __('messages.plowhorse'),
                                        'UNIT POTENSIAL' => __('messages.puzzle'),
                                        'UNIT KURANG BERKEMBANG' => __('messages.dog'),
                                        default => $advice['category']
                                    } }}
                                                </span>
                                            </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        @foreach([
                                            'UNIT UNGGULAN' => [
                                                'bg-emerald-500',
                                                __('messages.star_title'),
                                                __('messages.star_desc')
                                            ],
                                            'UNIT ANDALAN' => [
                                                'bg-amber-500',
                                                __('messages.plowhorse_title'),
                                                __('messages.plowhorse_desc')
                                            ],
                                            'UNIT POTENSIAL' => [
                                                'bg-indigo-500',
                                                __('messages.puzzle_title'),
                                                __('messages.puzzle_desc')
                                            ],
                                            'UNIT KURANG BERKEMBANG' => [
                                                'bg-rose-500',
                                                __('messages.dog_title'),
                                                __('messages.dog_desc')
                                            ],
                                        ] as $cat => $style)
                                        <div @click="activeFilter = activeFilter === '{{ $cat }}' ? 'ALL' : '{{ $cat }}'"

                                                               class="bg-white dark:bg-gray-900 
                            p-4 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm relative overflow-hidden group cursor-pointer transition-all duration-200"
            ...
            o                                -->
                                           ...
                                           y-between items-center px-1" x-show="activeFilter !== 'ALL'" x-transition>

                                                        <span class="text-s
                                           m text-gray-500">{{ __('messages.showing_category') }}: <strong x-text="activeFilter"></strong></
                                            span>
                    ...
                    <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
                        <div class="overflow-x-auto o
                                                   verflow-y-hidden">
                            <table class="w-full table-auto divide-y divide-gray-200 dark:divide-white/5 text-left">
                                <thead class="bg-gray-50 dark:bg-white/5">
                                    <tr>
                                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">{{ __('messages.product') }}</th>
                                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">{{ __('messages.category') }}</th>
                                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">{{ __('messages.popularity_qty') }}</th>
                                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">{{ __('messages.margin_profit') }}</th>
                                        <th class="px-4 py-3 text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">{{ __('messages.hpp_sell') }}</th>
                                    </tr>
                                </thead>
                                        ...
                                        default => $item['category']
                                                            ...
                                                            <td class="px-4 py-4 whitespace-nowrap">
                                                                <div class="text-[10px] text-gray-500">
                                                                    <div>{{ __('messages.hpp') }}: Rp{{ number_format($item['cogs'], 0, ',', '.') }}</div>
                                                                    <div class="font-bold text-gray-900 dark:text-white">{{ __('messages.selling_price') }}: Rp{{ number_format($item['sell_price'], 0, ',', '.') }}</div>
                                                                </div>
                                                            </td>
                                                        </tr>
                                    @endforeach
